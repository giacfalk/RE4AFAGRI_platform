...
if(field_size_contraint==T){field_size <- projectRaster(field_size, mask_raster_to_polygon(files[[1]], st_as_sfc(st_bbox(clusters_voronoi))), method = "bilinear") ; m <- field_size; m[m > 29] <- NA; field_size <- mask(field_size, m); files <- mask_raster_to_polygon(files, st_as_sfc(st_bbox(clusters_voronoi))); for (i in 1:nlayers(files)){crs(files[[i]]) <- crs(field_size)}; field_size <- projectRaster(field_size,files[[1]]); files <- mask(files, field_size)}

if(buffers_cropland_distance==T){clusters_buffers_cropland_distance <- projectRaster(clusters_buffers_cropland_distance,files[[1]], method = "ngb"); files <- mask(files, clusters_buffers_cropland_distance)}

field_size <- raster(find_it("field_size_10_40_cropland.img"))
field_size <- mask_raster_to_polygon(field_size, st_as_sfc(st_bbox(clusters)))
crs(field_size) <- as.character(CRS("+init=epsg:4236"))

if(field_size_contraint==T){field_size <- projectRaster(field_size, mask_raster_to_polygon(files2[[1]], st_as_sfc(st_bbox(clusters_voronoi))), method = "bilinear") ; m <- field_size; m[m > 29] <- NA; field_size <- mask(field_size, m); files2 <- mask_raster_to_polygon(files2, st_as_sfc(st_bbox(clusters_voronoi))); for (i in 1:nlayers(files2)){crs(files2[[i]]) <- crs(field_size)}; field_size <- projectRaster(field_size,files2[[1]]); files2 <- mask(files2, field_size)}

if(buffers_cropland_distance==T){clusters_buffers_cropland_distance <- projectRaster(clusters_buffers_cropland_distance,files2[[1]], method = "ngb"); files2 <- mask(files2, clusters_buffers_cropland_distance)}
...
for (timestep in planning_year){
  ...
  clusters <- clusters %>%  mutate(!!paste0("yield_",  names(files)[X], "_r_cp_", timestep) := (!!as.name(a)) * pull(!!aa[paste0("A_",  names(files)[X], "_r")]) * crop_processed_share_target * (match(timestep, planning_year) / length(planning_year)))
...
if (process_already_irrigated_crops==T){
  ...
  for (X in 1:nlayers(files)){
    ...
    clusters <- clusters %>%  mutate(!!paste0("yield_",  names(files)[X], "_i_cp_", timestep) := (!!as.name(a)) * pull(!!aa[paste0("A_",  names(files)[X], "_i")]) * crop_processed_share_target * (match(timestep, planning_year) / length(planning_year)))
...
for (X in as.vector(energy_crops[,1])){
  for (m in 1:12){
    ...
    clusters[paste0("n_machines_" , X , "_" , as.character(m), "_", timestep)] <- ceiling(pull(aa[paste0("kwh_cp" , X , "_" , as.character(m), "_", timestep)]) / ((energy_crops$kg_per_hour_kwmin[as.vector(energy_crops[,1]) == X]) * (sum(load_curve_cp>0))))
...
for (X in as.vector(energy_crops[,1])){
  ...
  clusters[paste0("n_machines_" , X,  "_",timestep)] <- as.vector(as.matrix(aa %>%  dplyr::select(starts_with(paste0("n_machines_" , X)) & contains(as.character(timestep)))) %>% rowMax(.) %>% as.numeric())
